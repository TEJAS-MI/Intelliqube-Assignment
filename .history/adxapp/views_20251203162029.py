...
def process_file(request):
    ...
    # -----------------------------------------------------------
    # FIX: convert string-formatted numbers to floats for plotting
    # -----------------------------------------------------------
    plot_adx = pd.to_numeric(result_df['ADX'], errors='coerce')
    plot_plus_di = pd.to_numeric(result_df['+DI14'], errors='coerce')
    plot_minus_di = pd.to_numeric(result_df['-DI14'], errors='coerce')

    # Produce plot (using numeric-converted values)
    plt.figure(figsize=(10, 5))
    plt.plot(plot_adx, label='ADX')
    plt.plot(plot_plus_di, label='+DI14')
    plt.plot(plot_minus_di, label='-DI14')
