...
def calculate_adx(input_df):
    ...
    # Ensure first TR cell is blank (Excel)
    if len(df) >= 1:
    ...
    # >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
    # REQUIRED FIX:
    # Row 1 blank, row 2 blank, row 3 onward → 0 instead of blank.
    # Excel shows 0 starting from 3rd data row.
    # >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>

    if len(df) > 2:
        # row index 0 and 1 remain blank
        df.loc[df.index[2]:, '+DM 1'] = df['+DM 1'].fillna(0)
        df.loc[df.index[2]:, '-DM 1'] = df['-DM 1'].fillna(0)
