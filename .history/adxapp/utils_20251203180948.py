...
def calculate_adx(input_df):
    ...
    # -----------------------------------------------------------------------
    # 🔥 CORRECTED FIX — Your file has an extra top row, so 0.00 must start at df.index[3]
    # Row1: blank
    # Row2: blank
    # Row3: blank (your actual file)
    # Row4+: 0.00
    # -----------------------------------------------------------------------
    for idx in df.index[3:]:
        if pd.isna(df.at[idx, '+DM 1']):
            df.at[idx, '+DM 1'] = 0.0
        if pd.isna(df.at[idx, '-DM 1']):
            df.at[idx, '-DM 1'] = 0.0
    # -----------------------------------------------------------------------
    ...
    # -----------------------------------------------------------------------
    # 🔥 FORCE ALL NUMERIC COLUMNS TO 2 DECIMALS (except NaN)
    # -----------------------------------------------------------------------
    numeric_cols = df.select_dtypes(include=[np.number]).columns
    df[numeric_cols] = df[numeric_cols].applymap(
        lambda x: np.nan if pd.isna(x) else float(f"{x:.2f}")
    )
