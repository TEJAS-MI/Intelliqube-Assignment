...
def calculate_adx(input_df):
    ...
    # First row must stay blank (Excel behavior)
...
# For all other rows:
    ...
    # --- FORCE ZERO VALUES TO DISPLAY EXACTLY LIKE EXCEL ---
    zero_columns = [
        'TR', '+DM 1', '-DM 1',
        'TR14', '+DM14', '-DM14',
        '+DI14', '-DI14',
    ...
    for col in zero_columns:
        if col in df.columns:
            df[col] = df[col].apply(
                lambda x: 0 if (isinstance(x, (int, float)) and abs(x) < 1e-12) else x
            )
