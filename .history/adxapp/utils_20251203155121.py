...
def wilder_sum_excel_style(series, n):
    ...
    # initial sum = sum of arr[1]..arr[n]
...
def calculate_adx(input_df):
    ...
    # Excel behavior: first TR row is blank
    if len(df) >= 1:
    ...
    # --------------------------------------------------
    # *** REQUIRED FIX: ROW 3 (index 1) must show 0 ***
    # --------------------------------------------------
    if len(df) > 1:
        if np.isnan(df.loc[df.index[1], '+DM 1']):
            df.loc[df.index[1], '+DM 1'] = 0
        if np.isnan(df.loc[df.index[1], '-DM 1']):
            df.loc[df.index[1], '-DM 1'] = 0

    # Row 4 onwards (index 2→end) already handled correctly
    if len(df) > 2:
    ...
    # Round values (optional: 4 decimals)
