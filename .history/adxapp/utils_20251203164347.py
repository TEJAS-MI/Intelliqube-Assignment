...
def calculate_adx(input_df):
   ...
   # Force Excel-style 0.00 in row-3 (index 2)
    df.loc[df.index[2], '+DM 1'] = 0
    df.loc[df.index[2], '-DM 1'] = 0

# And from row-4 onward, replace blanks with zero normally
    if len(df) > 3:
        df.loc[df.index[3]:, '+DM 1'] = df.loc[df.index[3]:, '+DM 1'].fillna(0)
        df.loc[df.index[3]:, '-DM 1'] = df.loc[df.index[3]:, '-DM 1'].fillna(0)
