...
# Crucial step: Use a non-GUI backend for plotting on servers (like Matplotlib 'Agg')
...
def process_file(request):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            # where the ADX smoothing begins to stabilize the chart.
            ax.plot(results_df.index[14:], results_df['ADX'].iloc[14:], label='ADX (14)', color='blue')
            ax.plot(results_df.index[14:], results_df['+DI14'].iloc[14:], label='+DI (14)', color='green')
            ax.plot(results_df.index[14:], results_df['-DI14'].iloc[14:], label='-DI (14)', color='red')
            
            ax.legend(loc='upper right')
            ax.set_title('ADX and Directional Indicators (N=14)')
            ax.set_ylabel('Value')
            ax.grid(True, alpha=0.5)
            ax.set_xticklabels([], minor=True) # Remove x-axis labels
